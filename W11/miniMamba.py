...
class MambaBlock(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=4 ,expand=2):
        ...
        # [修正 1] 修改輸出維度為 d_state * 3，以匹配 split 的需求
        # 原本是: self.d_state + self.d_model * 2
    ...
    def ssm(self, x):
        ...
        x_db1 = self.x_proj(x) # (B, L, self.d_state + self.d_model * 2)
        delta, B, C = x_db1.split([self.d_state, self.d_state, self.d_state], dim=-1)
